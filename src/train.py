...
cfg.base_net_weights = os.path.join('./model/', get_weight_path())
...
classifier = classifier(shared_layers, roi_input, cfg.num_rois, nb_classes=len(classes_count), trainable=True)
...
for epoch_num in range(num_epochs):
        ...
        while True:
            try:
                ...
                result = rpn_to_roi(P_rpn[0], P_rpn[1], cfg, K.image_dim_ordering(), use_regr=True,
                                                overlap_thresh=0.7,
                                                max_boxes=300)
                # note: calc_iou converts from (x1,y1,x2,y2) to (x,y,w,h) format
                X2, Y1, Y2, IouS = calc_iou(result, img_data, cfg, class_mapping)
